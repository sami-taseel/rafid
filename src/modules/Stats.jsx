  ...
  useEffect(() => {
    async function load() {
      ...
      const [students, sessions, activities, attendance, sanctions, support, responses,
        ...
        supabase.from('students').select('id, degree_level, profile_reviewed, admission_status, unit_id, persons(nationality)').or('is_test.is.null,is_test.eq.false'),
        ...
        supabase.from('attendance').select('status, students!inner(is_test)').or('is_test.is.null,is_test.eq.false', { foreignTable: 'students' }),
        ...
        supabase.from('attendance').select('status, sessions(planned_date), students!inner(is_test)').or('is_test.is.null,is_test.eq.false', { foreignTable: 'students' }),
        supabase.rpc('travelers_count'),
        supabase.from('students').select('admission_status').or('is_test.is.null,is_test.eq.false'),
