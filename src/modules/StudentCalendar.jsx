    ...
    <div className="st-cal">
      ...
      <div className="st-cal-upcoming">
        ...
        <div className="cc-grid">
          {monthActivities.map(s => <CompactCard key={s.id} session={s} studentId={studentId} attStatus={attMap[s.id]} targetType={typeMap[s.activity_id]} isMonitor={isMonitor} onAttChange={(sid, st) => setAttMap(m => ({ ...m, [sid]: st }))}
        ...
        <div className="confirm-overlay" onClick={() => setDaySel(null)}>
          <div className="confirm-box" onClick={e => e.stopPropagation()} style={{ textAlign: 'right', maxWidth: 460 }}>
            ...
            <div className="cc-grid" style={{ marginTop: 12 }}>
              {daySel.ss.map(s => <CompactCard key={s.id} session={s} studentId={studentId} attStatus={attMap[s.id]} targetType={typeMap[s.activity_id]} isMonitor={isMonitor} onAttChange={(sid, st) => setAttMap(m => ({ ...m, [sid]: st }))}
